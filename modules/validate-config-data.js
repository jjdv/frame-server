...
const { filePathNotEmpty } = require('./helpers-basic')
...
module.exports = function validateConfigData(config) {
    ...
    validateServerMiddlewares(config.serverMiddlewares, validationStatus)
    ...
    process.env.siteMiddlewaresDir = siteMiddlewaresDir ? siteMiddlewaresDir : serverRootDir
    ...
    validateWrongRequestHandlerDef(wrongRequestHandler, validationStatus)
...
function processView(view, processStatus) {
    ...
    if (view.engine) {
        if (typeof view.engine !== 'string')
            processStatus.reportErr('view.engine in the server config file must be a string, not: ', view.engine)
    }
    if (view.dir) {
