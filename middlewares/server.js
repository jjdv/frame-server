...
function validateServerMiddlewares(serverMiddlewares, status) {
    ...
    checkHelmet(serverMiddlewareNames, status)
...
function checkHelmet(serverMiddlewareNames, status) {
    const helmetIndex = serverMiddlewareNames.indexOf('helmet')

    if (helmetIndex == -1) {
        if (!serverConfig.noHelmet && !process.argv.includes('--no-helmet')) {
...
function checkPackageValidity(serverMiddlewareNames, status) {
    serverMiddlewareNames.forEach(mName => {
        if (!packageNames[mName]) status.reportErr('Invalid server middleware specification: ', mId)
    })
}
